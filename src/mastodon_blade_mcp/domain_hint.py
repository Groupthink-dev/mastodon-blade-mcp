...
BladeConfigStore (Convention #23) at::
    ...
    <state-root>/blade-config/mastodon-blade-mcp/config.yaml
...
class Pattern:
    ...
    Attributes:
        field: Logical field name (e.g. ``account_acct``, ``tags``,
            ``mentions``, ``content``, ``spoiler_text``); resolved by the
            per-blade ``field_projector`` callable into the record's
